...
class application_impl {
   public:
      ...
      fs::path                _data_dir{"data-dir"};
      fs::path                _config_dir{"config-dir"};
...
bool application::initialize_impl(int argc, char** argv, vector<abstract_plugin*> autostart_plugins) {
   ...
   if( options.count( "data-dir" ) ) {
      ...
      // Should be .as<fs::path>() but paths with escaped spaces break bpo e.g.
   ...
   if( options.count( "config-dir" ) ) {
      ...
      fs::path config_dir = workaround;
      if( config_dir.is_relative() )
         config_dir = fs::current_path() / config_dir;
      my->_config_dir = config_dir;
   }
   ...
   if (!fs::exists(my->_config_file_name)) {
