...
namespace appbase {
// adapted from:
// https://www.boost.org/doc/libs/1_69_0/doc/html/boost_asio/example/cpp11/invocation/prioritised_handlers.cpp
...
public:
  ...
  class executor {
  public:
    executor(execution_priority_queue& q, int p): context_(q), priority_(p) {}
...
private:
  ...
  public:
    queued_handler_base(int p, size_t order): priority_(p), order_(order) {}
  ...
  public:
    queued_handler(int p, size_t order, Function f): queued_handler_base(p, order), function_(std::move(f)) {}
  ...
  std::priority_queue<std::unique_ptr<queued_handler_base>, std::deque<std::unique_ptr<queued_handler_base>>,
    deref_less>
    handlers_;
